...
@register("loader", "hf_model")
class HFModelLoader(BaseStage):
    """Any HuggingFace checkpoint → capture-ready `(module, args)`.

    Sibling of `zoo_op`: same output contract, so the existing exporter /
    analyzer stages take it unchanged. Load repairs (meta weights) and capture
    rewrites (eager attention, forward-only) happen here, in the loader, because
    that is the layer they belong to — see `torch_mlir_zoo.capture`.
    """

    def __init__(self, model_id: str, task: str = "causal_lm", seq_len: int = 32):
        from .capture import prepare
        from .models.hf import TASKS, example_args, load_hf_model

        model = load_hf_model(model_id, task)
        self._args = example_args(model, task, seq_len)
        self._model, self.rewrites = prepare(model, arg_names=TASKS[task][1])
