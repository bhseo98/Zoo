...
차이는 **decomposition을 돌리느냐**다. sharktank는 GPU linalg codegen이 전제라
항상 분해하지만, 융합 op을 매치하는 하위 소비자를 겨냥할 때는 `profile="fused"`가
`run_decompositions()`를 건너뛰어 `aten.linear` / `rms_norm` / SDPA를 그대로 남긴다.
같은 turbine 경로에서 나온다 — 백엔드를 바꾸는 문제가 아니다. 근거는
[ARCHITECTURE.md §3.3](ARCHITECTURE.md#33-융합을-결정하는-것은-백엔드가-아니라-프로파일이다).
