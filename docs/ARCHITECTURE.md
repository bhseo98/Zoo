...
>
> 각 관점을 소스로 실측·교차검증. 레시피: [RECIPES.md](RECIPES.md).
...
생산하는 온디바이스 프론트엔드다. 선(line) 아래 계층 — NPU 런타임 컴파일러와
시뮬레이터 — 은 별도로 전달받는 부분으로, **이 저장소의 범위 밖**이다(내부 구현은
별도 관리). 두 절반은 **torch-MLIR 경계에서 합류**한다: 우리 zoo가 그 경계까지의
IR을 생산하고, 하위 런타임이 이를 그대로 소비한다.
...
| ④ NPU 런타임 (컴파일러 + 시뮬레이터) | 전달받음 | **이 저장소 범위 밖** — 우리 torch-dialect MLIR을 그대로 입력으로 받는 하위 스택. 내부 구현(코드 생성 · 마이크로커널 · 시뮬레이터)은 별도 관리 |
...
flowchart TB
  ...
  JOIN --> recv
  subgraph RECV["RECEIVED — NPU 런타임 (범위 밖 · 별도 관리)"]
    direction TB
    recv["NPU 런타임 컴파일러 + 시뮬레이터<br/>우리 torch-dialect MLIR을 그대로 소비"]
  end
...
| ② | PyTorch→MLIR lowering 프레임워크 | 계층 ①–③ + JOIN | ⚠ Whisper FP32 lowering 검증(이 repo); INT8 E2E는 별도 관리. Llama layer-0 합류 계약 확정 |
| ③ | 하위 NPU 런타임 (컴파일러 + 시뮬레이터) | JOIN 아래 계층 | 별도 관리 · **이 저장소 범위 밖** |

스택 전반에 고정된 불변식: **2 GB DRAM budget**(프로파일러가 budget 초과 로드에
경고, INT8 양자화는 명시적 budget 예외로 진입), **시뮬레이터 기반 검증**(실리콘
전 단계), **모델 swap 프레임워크**(한 번 빌드하고
...
> ⚠ 양자화 커널(`block_scaled_q8`)과 아래 실측 수치는 **별도 repo에서 관리** — 이
> 저장소만으로는 재현되지 않는다(설계·경로 설명 목적).
...
2. **합류 경로**: 우리는 f16 fused IR만 넘긴다. 그 뒤 양자화를 어떻게 처리할지는
   하위 런타임의 몫이며 이 저장소 범위 밖이다.

## 3.5 합류 계약 (op 레벨)

합류 = torch-dialect 경계. 하위 런타임은 현재 단일-layer 스텁을 매치하고, 캡스톤은
그 자리에 우리 `LlamaOnDevice` layer를 투입한다. op-레벨 계약은 다음과 같다:
...
flowchart TD
    ...
    BT --> ANA
    AT --> JOIN["JOIN: 하위 런타임이 fused aten.linear를<br/>매치 (내부 구현은 범위 밖)"]
    JOIN --> UK["하위 NPU 런타임 (범위 밖 · 별도 관리)"]
    BT -. "mm/bmm: no linear pattern" .-> XJOIN["join fails"]
    BT --> Q["quantize_linears_<br/>block_scaled_q8 (INT8, last step)"]
...
| 두 export 백엔드 | 우리 | ✅ | zoo + turbine export 테스트 통과 (29 passed / 3 skipped) |
| **합류 백엔드 결정** | 우리 | ✅ | `torch_mlir.compile`이 `aten.linear` 보존 (turbine 분해는 실측) |
| 캡스톤 계약-layer export | 우리 | ✅ 템플릿 | 계약 shape decoder-layer export 템플릿 (join backend = torch_mlir) |
| INT8 `block_scaled_q8` | 별도 repo | ⤴ 이 저장소 밖 | 양자화 커널은 별도 관리 |
